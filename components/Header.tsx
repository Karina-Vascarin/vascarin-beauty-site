...
export default function Header() {
  ...
  // Esta função só é chamada no submit (Enter ou clique na lupa)
  const handleSearch = async (e: React.FormEvent) => {
    e.preventDefault();
    const query = searchTerm.trim();
    if (!query) return;

    // 1. Busca os produtos atuais para verificar se o que ela digitou existe (para marcar resultados)
    // Assumimos que o fetch de produtos pode ser feito ou apenas registramos a intenção
    try {
      ...
      await supabase.from('buscas_site').insert([{
        ...
        resultados: 0 // Você pode implementar lógica para checar se encontrou algo antes de redirecionar
      }]);
    } catch (error) {
      console.error("Erro ao registrar busca:", error);
    }

    // 2. Redireciona para a página de resultados com a query
  ...
  return (
    <header className="w-full bg-white border-b border-gray-200 py-3 px-6 sticky top-0 z-50">
      <div className="max-w-7xl mx-auto flex items-center justify-between gap-6">
        ...
        {/* Formulário com onSubmit, só dispara ao dar ENTER ou clicar na lupa */}
        <div className="flex-1 max-w-xl hidden md:block">
          <form onSubmit={handleSearch} className="relative flex items-center">
            ...
            />
            <button type="submit" className="absolute right-3 text-gray-400 hover:text-black transition-colors cursor-pointer">
